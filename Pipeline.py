...
def Multiline_detection(image):
    ...
    for (lower, upper) in boundaries:
        try:
            ...
            sum_of_individual_colors.append(0)
...
def Segmentation(individual_lines):
    ...
    for line in individual_lines:
        if line.any():
            ...
            cont, _  = cv2.findContours(line, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def Post_processing(max_prob_list):
  ...
  template_list = ["^[A-Z]{2}[0-9]{2}[A-Z]{2}[0-9]{4}$","^[A-Z]{2}[0-9]{2}[A-Z]{3}[0-9]{3}$","^[A-Z]{2}[0-9]{2}[A-Z]{2}[0-9]{3}$","^[A-Z]{2}[0-9]{1}[A-Z]{2}[0-9]{4}$"]
  ...
  for key, value in valid_numberplates.items():
    ...
    if count< topN:
      if pattern == 1 :
        ...
        print(number_plate+"   "+"Confidence ="+f"{str(round(accuracy,3)): <6}"+"%"+"    "+"Match = True")
...
image = cv2.imread(r'C:\Users\SIDDHI\Downloads\Airpix_ANPR-Aniket\Airpix_ANPR-Aniket\lp2.jpg')
...
individual_lines = Multiline_detection(img)

""" for line in individual_lines:
  cv2.imshow("Individual line",line)
  cv2.waitKey(-1) 
  cv2.destroyAllWindows()"""

crop_characters, character_width = Segmentation(individual_lines)
...
crop_characters = Splitting_Characters(crop_characters,character_width)
...
model, label_encoder = Load_ocr_model()

max_prob_list = Perform_ocr(crop_characters, model, label_encoder)

Post_processing(max_prob_list)
